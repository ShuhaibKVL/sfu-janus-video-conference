...
export default function VideoLayout({
  ...
  const hasExtraUsers = remoteStreams.length > MAX_VISIBLE_STREAMS;
  ...
  const visibleRemotes = useMemo(() => {
    ...
    if (prioritySpeakerId) {
      const hiddenSpeaker = streams.find((s) => s.id === prioritySpeakerId);

      const alreadyVisible = streams
        .slice(0, MAX_VISIBLE_STREAMS)
        .some((s) => s.id === prioritySpeakerId);

      /**
       * Move hidden speaker to front
       */
      if (hiddenSpeaker && !alreadyVisible) {
        streams = [
          hiddenSpeaker,
          ...streams.filter((s) => s.id !== prioritySpeakerId),
        ];
      }
    }

    return hasExtraUsers
      ? streams.slice(0, MAX_VISIBLE_STREAMS - 1)
      : streams.slice(0, MAX_VISIBLE_STREAMS);
  }, [remoteStreams, hasExtraUsers, prioritySpeakerId]);
            ...
            <div className="max-w-7xl mx-auto h-full flex flex-col">
              ...
              >
                ...
                <div className="w-[280px] flex flex-col gap-4">
                  {visibleRemotes
                    .filter((remote) => remote.id !== sharedScreen.id)
                    .slice(0, 2)
                    .map((remote) => {
                      return (
                        <div
