...
export async function POST() {
  try {
    /*
        ...
        */
    const createSessionRes = await fetch(JANUS_HTTP_URL, {
      method: "POST",
      headers: {
    ...
    const attachPluginRes = await fetch(`${JANUS_HTTP_URL}/${sessionId}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    const pluginData = await attachPluginRes.json();

    const handleId = pluginData?.data?.id;
    ...
    const roomId = roomData?.plugindata?.data?.room;

    if (!roomId) {
      throw new Error("Failed to get created room ID");
    }
