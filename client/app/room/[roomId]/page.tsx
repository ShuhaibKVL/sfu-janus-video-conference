...
export default function RoomPage() {
  ...
  const [remoteStreams, setRemoteStreams] = useState<
    { id: string; stream: MediaStream; name: string; isCameraOff: boolean }[]
  >([]);
  const [prioritySpeakerId, setPrioritySpeakerId] = useState<string | null>(
    null,
  );
  const [visibleStreamIds, setVisibleStreamIds] = useState<string[]>([]);
  ...
  const [sharedScreen, setSharedScreen] = useState<{
    id: string;
    stream: MediaStream;
    name: string;
  } | null>(null);
  ...
  const speakerStreams = remoteStreams.map((remote) => ({
  ...
  useEffect(() => {
     ...
     * Is already visible?
     */
    const isAlreadyVisible = visibleStreamIds.includes(activeSpeakerId);
    ...
    if (now - lastDominantSwitchRef.current < SWITCH_COOLDOWN) {
      return;
    }
  ...
  const subscribeToPublisher = (publisherId: number, publisherName: string) => {
    ...
    janusRef.current.attach({
      ...
      success: (pluginHandle: any) => {
        console.log(
          "Attached to plugin for subscribing with handle ID:",
          pluginHandle.getId(),
        );
      ...
      onremotetrack: (track: MediaStreamTrack, mid: string, on: boolean) => {
        console.log(
          "Received remote track for subscriber:",
          track,
          "mid:",
          mid,
          "on:",
        ...
        if (on) {
          ...
          const isScreenShare = publisherName.endsWith("'s screen");
  ...
  const startJanus = async () => {
    ...
    janusRef.current = new Janus({
      ...
      success: async () => {
        const mediaStream = await createLocalMediaStream();
        ...
        janusRef.current.attach({
          ...
          success: (pluginHandle: any) => {
            console.log(
              "Attached to plugin with handle ID:",
              pluginHandle.getId(),
            );
          ...
          onmessage: (msg: any, jsep: any) => {
            ...
            if (msg.videoroom === "event" && msg.leaving) {
              ...
              subscribedFeeds.current.delete(Number(msg.leaving));
      ...
      for (const subscriber of subscriberRefs.current.values()) {
        try {
          ...
          subscriber.detach({
            ...
            error: (err: any) => {
              console.error("Subscriber detach error:", err);
            },
          });
        } catch (error) {
          console.error(error);
  ...
  useEffect(() => {
    ...
    notificationAudioRef.current = new Audio(
      "/sounds/soundreality-notification-center-443093.mp3",
    );
  ...
  useEffect(() => {
    subscriberRefs.current.forEach((subscriber, publisherId) => {
      const publisherIdStr = publisherId.toString();

      const isVisible = visibleStreamIds.includes(publisherIdStr);

      const isDominant = prioritySpeakerId === publisherIdStr;
  ...
  }, [visibleStreamIds, prioritySpeakerId]);
  ...
  const startScreenShare = async () => {
    try {
      ...
      janusRef.current.attach({
        ...
        success: (pluginHandle: any) => {
          console.log(
            "Attached to plugin for screen sharing with handle ID:",
            pluginHandle.getId(),
          );
